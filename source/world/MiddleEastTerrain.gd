...
func _continuous_macro_color(height_m: float, lon: float, lat: float) -> Color:
	...
	base = base.lerp(Color(0.26, 0.25, 0.22, 1.0), southern_basalt * 0.68)
	var northern_steppe := clampf((lat - 35.9) / 0.9, 0.0, 1.0)
	base = base.lerp(Color(0.37, 0.50, 0.23, 1.0), northern_steppe * 0.40)
	var central_fertile := clampf(1.0 - absf(lon - 36.75) / 0.85, 0.0, 1.0) * clampf(1.0 - absf(lat - 35.15) / 1.55, 0.0, 1.0)
	base = base.lerp(Color(0.32, 0.47, 0.22, 1.0), central_fertile * 0.26)
	var interior_plateau := clampf((lon - 37.25) / 1.25, 0.0, 1.0) * clampf((39.35 - lon) / 1.15, 0.0, 1.0)
	base = base.lerp(Color(0.59, 0.47, 0.27, 1.0), interior_plateau * 0.32)
	return base
...
func _build_art_directed_battlefield() -> void:
	...
	for village_index in range(15):
		var vx := -3.90 + float(village_index % 5) * 0.25
		var vz := 1.02 + float(int(village_index / 5)) * 0.24
	...
	for hamlet_index in range(6):
		...
		var hh := 0.040 + 0.010 * float(hamlet_index % 2)
		_append_art_box(buildings, _art_point(hx, hz, 0.010), 0.11, 0.09, hh)
		building_count += 1
...
func _first_crossed_legal_crossing(start: Vector2, destination: Vector2) -> Dictionary:
	var best: Dictionary = {}
	var best_t := INF
	...
	for raw_feature in _hydrology_data.get("rivers", []):
		...
		for i in range(points.size() - 1):
			...
			for raw_crossing in _crossings_for_river(river_group):
				...
				var crossing: Dictionary = raw_crossing
				var crossing_geo := Vector2(float(crossing.get("lon", 0.0)), float(crossing.get("lat", 0.0)))
				var distance := _geo_distance_km(point, crossing_geo)
				if distance > 0.45:
					continue
				if t < best_t or (is_equal_approx(t, best_t) and distance < best_distance):
					best_t = t
...
func plan_ground_route(start: Vector2, destination: Vector2) -> Array[Vector2]:
	...
	var current := start
	for _hop in range(8):
		var legal := _first_crossed_legal_crossing(current, final_target)
		if not legal.is_empty():
			var legal_crossing: Dictionary = legal.get("crossing", {})
			var legal_path := _crossing_route_points(legal_crossing, current)
			if legal_path.is_empty():
				return []
			var legal_advanced := false
			for legal_point in legal_path:
				if _geo_distance_km(current, legal_point) < 0.002:
					continue
				route.append(legal_point)
				current = legal_point
				legal_advanced = true
			if not legal_advanced:
		...
		if crossing.is_empty():
			return []
		var crossing_path := _crossing_route_points(crossing, current)
		if crossing_path.is_empty():
			return []
		var advanced := false
		for crossing_point in crossing_path:
			if _geo_distance_km(current, crossing_point) < 0.002:
				continue
			route.append(crossing_point)
...
func select_nearest_radar_target_uv(uv: Vector2, max_distance: float = 0.06) -> bool:
	var best := _nearest_radar_item(uv, max_distance, true)
	if best.is_empty():
		best = _nearest_radar_item(uv, max_distance, false)
	if best.is_empty():
		return false
...
func _handle_world_tap(screen_position: Vector2) -> void:
	...
	if closest_index >= 0:
		if not _selected_unit_indices.is_empty() and closest_index not in _selected_unit_indices:
			if issue_selected_attack(closest_index) > 0:
				_update_status()
				return
		if _rts_zoom_level >= RTS_DETAIL_UNIT_LOD_MIN:
...
func select_previous_unit() -> bool:
	if _units.is_empty():
		clear_selected_units()
		return false
